...
local function get_niri_monitors()
  ...
  if not success or not monitors then return {} end
...
local function process_workspace_data()
  ...
  if err then return previous_workspace_state or {} end

  local success, workspaces = pcall(function()
    return cjson.decode(out)
  end)

  if not success or not workspaces then return previous_workspace_state or {} end

  local state_hash = table.concat(map(workspaces, function(w)
...
local function WorkspaceButton(props)
  return Widget.Button({
    ...
    on_clicked = function()
      local cmd = string.format("niri msg action switch-to-workspace-index %d %s", props.id - 1, props.monitor_name)
      astal.exec(cmd)
    end,
  })
end
  ...
  return Widget.Box({
    ...
    bind(workspaces):as(function(ws)
      ...
      for _, monitor in ipairs(ws) do
        table.insert(monitors, MonitorWorkspaces({
