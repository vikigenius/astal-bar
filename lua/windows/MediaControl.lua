    ...
    return Widget.Box({
        ...
        Widget.Label({
            ...
            label = bind(player, "artist"):as(function(artist)
                -- Handle Spotify's artist format (sometimes comes as table)
                if type(artist) == "table" then
                    return table.concat(artist, ", ")
                end
...
function MediaControlWindow.new(gdkmonitor)
    ...
    local function get_active_player()
        ...
        -- Prioritize Spotify by checking both old and new MPRIS bus names
        for _, player in ipairs(players) do
            if player.available then
                if player.bus_name:match("spotify") or player.bus_name:match("Spotify") then
                    return player
                end
            end
        end
