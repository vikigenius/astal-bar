...
local function find_desktop_entry(name)
  ...
  for _, app in ipairs(app_list) do
    if app and app.entry and (
          app.name and app.name:lower():match(name:lower()) or
          app.entry:lower():match(name:lower())
        ) then
      return app.entry
    end
  end
...
function M.update_running_apps()
  ...
  for _, window in ipairs(windows) do
    if window.app_id then
      for _, app in ipairs(app_list) do
        if app and app.entry and (
              app.entry:lower():match(window.app_id:lower()) or
              (app.wm_class and app.wm_class:lower():match(window.app_id:lower()))
            ) then
